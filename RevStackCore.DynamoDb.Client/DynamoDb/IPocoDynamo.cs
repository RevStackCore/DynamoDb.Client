...
namespace RevStackCore.DynamoDb.Client
{
    ...
    public interface IPocoDynamo : IPocoDynamoAsync, IRequiresSchema
    {
        ...
        QueryExpression<T> QueryIndex<T>(Expression<Func<T, bool>> keyExpression = null);
        QueryExpression<T> QueryIndex<T>(DynamoGlobalIndex index,Expression<Func<T, bool>> keyExpression = null);
        QueryExpression<T> QueryIndex<T>(DynamoLocalIndex index, Expression<Func<T, bool>> keyExpression = null);
        DynamoDbResult<T> ScanPage<T>(ScanExpression<T> request, int limit, Dictionary<string, AttributeValue> lastEvaluatedKey = null);
        DynamoDbResult<T> QueryPage<T>(QueryExpression<T> request, int limit, Dictionary<string, AttributeValue> lastEvaluatedKey = null);
        QueryExpression<T> QueryComposite<T>(Expression<Func<T, bool>> keyExpression = null);
        T DeleteByItem<T>(T item);
