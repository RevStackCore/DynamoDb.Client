...
using RevStackCore.Aws;
using RevStackCore.Aws.Support;
using ServiceStack;
...
namespace RevStackCore.DynamoDb.Client
{
    public partial class PocoDynamo : IPocoDynamo
    {
        ...
        public T GetItemFromQuery<T>(string tableName,string hashKey, object hashKeyValue)
        {
            string keyExpression = $"({hashKey}=:k0)";
            var result = QueryKey<T>(keyExpression, tableName, ":k0", hashKeyValue).Exec(1);
            if(result.Any())
            {
        ...
        {
            var isG = typeof(T).IsGlobalIndex();
        ...
        public QueryExpression<T> QueryIndex<T>(DynamoLocalIndex index, Expression<Func<T, bool>> keyExpression)
        {
            ...
            {
                string hashKey = keyExpression.ToBinaryProperty();
                string prop = index.Property;
                string typeName = typeof(T).Name;
                indexName = typeName.ToIndexName(IndexType.Local, prop, hashKey);
            }
            var q = new QueryExpression<T>(this)
            {
                IndexName = indexName,
        ...
        public QueryExpression<T> QueryIndex<T>(Expression<Func<T, bool>> keyExpression)
        {
            string prop = keyExpression.ToBinaryProperty();
            string indexName = typeof(T).ToGlobalIndexName<T>(prop);
        ...
        public QueryExpression<T> QueryKey<T>(string keyExpression, string tableName,string key, object value)
        {
            var q = new QueryExpression<T>(this)
            {
                TableName=tableName,
            ...
            args.Add(key, value);
            q.KeyCondition(keyExpression, args);
